...
mafengwoSession.cookies = cookielib.LWPCookieJar(filename="./cookie/mafengwoCookies.txt")
...
def mafengwoLogin(account, password):
    ...
    # 使用session直接post请求
    responseRes = mafengwoSession.post(postUrl, data=postData, headers=header, verify=False)
...
def isLoginStatus():
    ...
    responseRes = mafengwoSession.get(routeUrl, headers=header, allow_redirects=False, verify=False)
...
if __name__ == "__main__":
    ...
    if isLogin == False:
        ...
        mafengwoLogin("13184124312", "sdfsds")
