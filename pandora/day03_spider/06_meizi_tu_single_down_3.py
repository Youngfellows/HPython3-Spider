...
def read_url_for_json():
    ...
    for meinv_item in meinu_dict:
        ...
        # 下载首页大图
        try:
            urllib.request.urlretrieve(tupian_url, "./images/bigpic/" + str(1) + '.jpg')
            print('正在保存,第%s张图片' % (1))
        except urllib.error.HTTPError as e:
            print("靠,无法下载第{}张".format(1))
        ...
        for page in range(2, int(count) + 1):
            ...
            try:
                # 下载每页大图
                urllib.request.urlretrieve(tupian_url, "./images/bigpic/" + str(page) + '.jpg')
                print('正在保存,第%s张图片' % (page))
            except urllib.error.HTTPError as e:
                print("靠,无法下载第{}张".format(page))
