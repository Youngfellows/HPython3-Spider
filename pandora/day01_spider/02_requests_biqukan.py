...
class download(object):
    ...
    Returns:
        novel_name + '.novel' - 保存的小说名(string)
        numbers - 章节数(int)
    ...
    def get_download_url(self):
        charter = re.compile(r'[第弟](.+)[章张]', re.IGNORECASE)
        # charter = re.compile(r'[第弟](.+)[章张]', re.IGNORECASE)
        ...
        #print("novel_name:{}".format(novel_name))

        begin_flag = False
        numbers = 1
        children_list = download_soup.dl.children
        for child in children_list:
            ...
            if child != '\n':
                ...
                if begin_flag == True and child.a != None:
                    ...
                    # 正则匹配
                    # matchOjb = re.match(r'^[第弟](.+)[张]{1}\s', download_name
                    matchOjb = charter.match(download_name)
                    #print("{0},matchOjb = {1}".format(download_name, matchOjb))
                    if matchOjb != None:
                        names = str(download_name).split('章')
                        names2 = str(download_name).split('张')
                        name = charter.findall(names[0] + '章')
                        if names2[0] != str(download_name):
                            name = charter.findall(names2[0] + "张")
                            #print("1 names: {},names2: {}".format(names, names2))
                            names = names2
                        print("names: {},names2: {}".format(names, names2))
                        print("name: {}".format(name))
                        if name:
                            download_dict['第' + str(numbers) + '章 ' + names[1]] = download_url
                            numbers += 1
        return "./novel/" + novel_name + '.txt', numbers, download_dict
    ...
    def Downloader(self, url):
        ...
        soup_text = BeautifulSoup(str(texts), 'lxml').div.text.replace('\xa0', '').replace("app2();","")
        return soup_text
